...
function genItem({item, index, data, Entity}, h, host) {
  ...
  debug('genItem', name, value, ComponentType);
...
let FormBuilder = {
  ...
  beforeMount() {
    debug('lifecycle beforeMount', ...arguments);
  },
  mounted() {
    debug('lifecycle mounted', ...arguments);
  },

  beforeUpdate() {
    debug('lifecycle before update', ...arguments);

  },
  updated() {
    debug('lifecycle updated', ...arguments);
  },
  beforeDestroy() {
    debug('lifecycle before destroy', ...arguments);
  },
  destroyed() {
    debug('lifecycle destroyed', ...arguments);
  },
  watch: {
    'data': function(data) {
...
FormBuilder.register = function(name, component) {
  debug('form.register', name, component);
  if (!datatypes[name]) {
